...
for (const video of videos) {
  ...
  const date = (new Date(info.primary_info.published.text)).toISOString().substring(0, 10)
  if (date < cutOffDate) {
    continue
  }
  ...
  const description = emojiStrip(info.basic_info.short_description).trim().substring(0, 999)
  ...
  const activity = {
    ...
    "isPrivate": false,
    "targetAudience": ["Developer", "Technical Decision Maker", "IT Pro", "Student"],
    "tenant": "MVP",
    "title": title,
    "url": url,
    "reach": reach,
